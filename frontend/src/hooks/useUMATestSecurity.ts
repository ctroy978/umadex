...
interface UseUMATestSecurityProps {
...
export function useUMATestSecurity({ testAttemptId, isActive, onWarning, onLock }: UseUMATestSecurityProps) {
  ...
  const handleSecurityViolation = useCallback(async (violationType: string) => {
    ...
    try {
      const response = await umatestApi.logSecurityIncident(testAttemptId, {
  ...
  useEffect(() => {
    if (isActive && testAttemptId) {
      umatestApi.getSecurityStatus(testAttemptId).then(status => {
