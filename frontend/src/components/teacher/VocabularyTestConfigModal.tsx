...
import { XMarkIcon, InformationCircleIcon, Cog6ToothIcon } from '@heroicons/react/24/outline'
...
export default function VocabularyTestConfigModal({
  ...
  const handleSave = async () => {
    try {
      ...
      // Validate settings
      if (config.chain_enabled && config.weeks_to_include < 1) {
        setError('Weeks to include must be at least 1')
        return
      }
      
      if (config.questions_per_week < 3 || config.questions_per_week > 8) {
                        ...
                        <label className="flex items-start p-4 border rounded-lg cursor-pointer hover:bg-gray-50">
                          ...
                          <div className="ml-3">
                            ...
                            <div className="text-sm text-gray-600">
                              Include questions from previous vocabulary assignments to reinforce learning
                            </div>
                          </div>
                        </label>
